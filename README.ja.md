...
- 🖱️ **Cursor も**
  ...
  ポップオーバーがその旨を出すので、0 円を「使っていない」と読み違えません。プランの枠内で
  請求が発生しない利用（ダッシュボードの Cost 列が「-」の分）は実際に請求されないので、
  金額に足しません。金額欄が読めないイベントは、ローカル経路と同じように Cursor 公式の
  価格表から金額を出します。価格表に無いモデルは当て推量の単価を作らず 0 円のままです。
  サインイン切れが
...
- 🎨 **外観**
  ...
  設定でシステム / ライト / ダークを選べます。ポップオーバー、設定、About に効き、
  「システム」は macOS の外観に追従します。
